...
import EsApps from "../TabsContent/app"
		...
		<EsTabsContainer>
			<TabsHeader>
				{emailTabsData.map(tab => (
					<KButton key={tab.id} bc="transparent" h="27px" p="0px 0px" bm={tabvalue === tab.id ? "3px solid #FF7900" : null} onClick={() => setTabvalue(tab.id)}>
						<KreativeP fw={tabvalue === tab.id ? "bold" : "normal"} color={tabvalue === tab.id ? AppColors.brandColor : AppColors.muted}>
							{tab.label}
						</KreativeP>
					</KButton>
				))}
